 ...
 * @brief FINAL SKETCH: Corrected to use 3-channel (RGB565) input, detection threshold lowered,
 * and the classification logic is updated from Person/No-Person to **Door Closed/Door Open**.
 * ---------------------------------------------------------------------------------
 * --- UPDATES APPLIED ---
 * 1. Logic Change: Classification updated to **DOOR_CLOSED** and **DOOR_OPEN**.
 * 2. Constants Updated: PERSON/NO_PERSON indices changed to DOOR_CLOSED/DOOR_OPEN indices.
 * 3. Comments/Prints: Updated throughout the code to reflect the new task.
 * ---------------------------------------------------------------------------------
 */
...
#include "esp_log.h" 
// You must ensure 'model.h' containing 'door_status_model' is in your project directory
#include "model.h" 

// --- CONSTANTS AND CONFIGURATION ---
const int kTensorArenaSize = 384 * 1024; 
// ASSUMPTION: DOOR_CLOSED is index 0 and DOOR_OPEN is index 1 in the model's output.
#define DOOR_OPEN_INDEX 1 
#define DOOR_CLOSED_INDEX 0
#define DETECTION_THRESHOLD 0.70 // Sensitivity for positive detection (DOOR OPEN)
...
void setup() {
    ...
    model = tflite::GetModel(door_status_model); // Using the model name as defined in your prompt
...
void RunInferenceAndReport() {
    ...
    float door_closed_score = (float)(output->data.int8[DOOR_CLOSED_INDEX] - output->params.zero_point) * output->params.scale;
    ...
    Serial.printf("DEBUG: Raw Scores: Closed=%d, Open=%d\n", 
                  output->data.int8[DOOR_CLOSED_INDEX], 
                  output->data.int8[DOOR_OPEN_INDEX]);
    // --- DEBUG PRINTS END ---

    // 4. Report Results
    // Check DOOR_OPEN score against the threshold
    if (door_open_score >= DETECTION_THRESHOLD) {
        Serial.printf("🚪 **DOOR OPENED DETECTED!** Confidence: %.2f (Threshold: %.2f)\n", 
                      door_open_score, DETECTION_THRESHOLD);
    } else {
        // Door is considered closed if the open score is below the threshold
        Serial.printf("🔒 Door Closed. Highest 'Open' Score: %.2f ('Closed' Score: %.2f)\n", 
                      door_open_score, door_closed_score);
    }
}
